...
export default function Home() {
  ...
  const handleSendMessage = async (message) => {
    ...
    try {
      // hitting the chat endpoint of the backend
      ...
      setMessages((prev) => [
        ...prev,
        // In case of an error this is the response
  ...
  // When Clear is clicked, Messages are set to empty array and a new uuid is set.
  ...
  const handleDeleteSession = async (id) => {
    ...
    if (id === sessionId) {
      // if current session is deleted, we start a new one.
      handleClearSession()
    }
  ...
  // Loads history of previos sessions
  const handleLoadSession = async (session) => {
    setSessionId(session.id)
    setShowHistory(false)
  
    try {
      // history endpoint is hit
